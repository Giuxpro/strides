...
import { getOnboardingScreenById } from '@strides/db'
...
export default async function EditOnboardingScreenPage({ params }: Props) {
  const supabase = createClient()
  const { data: screen } = await getOnboardingScreenById(supabase, params.screenId)
  if (!screen) notFound()
  ...
  return (
    <div className="h-screen overflow-hidden flex flex-col">
      <div className="flex items-center gap-3 px-4 py-2 bg-gray-950 border-b border-gray-800 shrink-0">
        <a href="/admin/onboarding" className="text-xs text-gray-500 hover:text-gray-300 transition-colors">
          ← Onboarding
        </a>
