...
export function DragWordQuestion({ item, allVocab, moduleConfig, onAnswer, autoPlay = true }: EvalFormatProps) {
  ...
  function onPointerUp(e: React.PointerEvent) {
    ...
    if (inZone(e.clientX, e.clientY)) {
      ...
      if (correct) speak(vocab.text_en)
    ...
    <div className="flex flex-col items-center gap-4 w-full">
      ...
      <div className="flex gap-2 flex-wrap justify-center">
        {tiles.map(t => {
          ...
          const placed = result !== null && t.id === vocab.id
          return (
            <button
              key={t.id}
              ...
              style={{
                ...
                border: `2px solid ${placed ? '#22c55e' : 'var(--kids-border-color)'}`,
                color: 'var(--kids-text)',
