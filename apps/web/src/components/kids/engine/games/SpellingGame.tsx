...
export function SpellingGame({ items, onComplete, onBack, moduleConfig, progress }: Props) {
  ...
  const [tiles, setTiles] = useState<string[]>(() => shuffle([...letters]))
  const [slots, setSlots] = useState<(string | null)[]>(() => Array(letters.length).fill(null))
  const [shaking, setShaking] = useState(false)
  const [burst, setBurst] = useState(false)
  const [slotStates, setSlotStates] = useState<SlotState[]>(() => Array(letters.length).fill('empty'))
  const [usedTileIdxs, setUsedTileIdxs] = useState<Set<number>>(new Set())

  const resetForWord = useCallback((wordLetters: string[]) => {
    setTiles(shuffle([...wordLetters]))
    setSlots(Array(wordLetters.length).fill(null))
    setSlotStates(Array(wordLetters.length).fill('empty' as SlotState))
      ...
      <main className="relative z-10 flex-1 flex flex-col items-center justify-center px-4 sm:px-6 gap-5 sm:gap-8 py-4">
        ...
        <div className="flex flex-col items-center gap-2 sm:gap-3">
          ...
          <div className="relative">
            ...
            {burst && (
              <div
                className="absolute inset-x-0 flex justify-center animate-pop-in pointer-events-none"
                style={{ bottom: '-20px' }}
              >
