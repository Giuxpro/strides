...
/**
 * VowelsOrderGame — Asocia la vocal con su imagen
 *
 * TOP:    slots con solo el emoji del animal (sin letra — el niño debe razonar)
 * BOTTOM: cartas con letra + nombre del animal (sin imagen)
 * Drag:   arrastra la carta a la imagen que le corresponde
...
export function VowelsOrderGame({ items, onComplete, onBack, moduleConfig, progress }: Props) {
  const { reportCorrect, reportWrong, isTerminated } = useGameEvents()
  const speakFn = useSpeak()

  // Una sola ronda — 5 vocales, una pasada
  const [imageOrder] = useState(() => shuffle([...VOWEL_DEFS]))
  ...
  return (
    <div className="relative min-h-screen flex flex-col overflow-x-hidden">
        ...
        <div className="flex items-center gap-3 sm:gap-4">
          ...
          <div>
            ...
            <p className="font-bold text-base sm:text-lg" style={{ color: 'var(--kids-text)' }}>
              {placedCount}/{VOWEL_DEFS.length} encontradas
            </p>
          </div>
        </div>
      ...
      <main
        ref={containerRef}
        className="relative z-10 flex-1 flex flex-col items-center justify-center px-4 sm:px-6 gap-5 sm:gap-6 py-6 sm:py-8"
        style={{ touchAction: 'none' }}
      >
        <p className="text-sm sm:text-base font-semibold text-center" style={{ color: 'var(--kids-text-faint)' }}>
          {burst ? '¡Perfecto! 🎉' : 'Arrastra cada letra a la imagen que le corresponde'}
        </p>

        {/* TOP — image-only slots (scrambled order) */}
          ...
          {imageOrder.map(def => {
            ...
            return (
              <div
                ...
                className="flex flex-col items-center justify-between rounded-2xl transition-all duration-200"
                style={{
                  ...
                  boxShadow: isWrong
                    ...
                    : isPlaced
                      ...
                      : 'none',
                  animation: isWrong ? 'vo-shake 0.4s ease' : undefined,
                  padding:   isPlaced ? '8px 6px 6px' : '0',
                  gap:       isPlaced ? '2px' : '0',
                }}
              >
                {isPlaced ? (
                  // Placed: full card (letter + emoji + name)
                  <>
                  ...
                  // Empty: only the emoji — no letter hint
                  <div className="w-full h-full flex items-center justify-center">
                    <span style={{ fontSize: 'clamp(30px, 9vw, 46px)', lineHeight: 1 }}>{def.emoji}</span>
                  </div>
                )}
              </div>
            )
        ...
        {/* Divider */}
        <div className="w-full flex items-center gap-3" style={{ maxWidth: 400 }}>
        ...
        {/* BOTTOM — letter + name cards (no emoji) */}
        <div className="flex gap-2 sm:gap-3 justify-center flex-wrap">
          {poolOrder.map(def => {
            const isPlaced  = !!placed[def.letter]
            ...
            return (
              ...
              >
                {!isPlaced && (
                  <>
                    {/* Big letter — the answer to find */}
                    <span className="font-extrabold text-white leading-none"
                      style={{ fontSize: 'clamp(26px, 7.5vw, 40px)', textShadow: '0 2px 6px rgba(0,0,0,0.25)' }}>
                      {def.letter}
                    </span>
                    {/* Animal name — the hint */}
                    <span className="font-bold rounded-lg w-full text-center truncate px-1.5 py-0.5"
                      style={{ fontSize: 'clamp(8px, 2.3vw, 11px)', background: 'rgba(255,255,255,0.88)', color: '#374151' }}>
                      {def.animal}
                    </span>
                  </>
                )}
      ...
      {drag && dragDef && containerRef.current && (() => {
        ...
        return (
          ...
          >
            <span className="font-extrabold text-white leading-none"
              style={{ fontSize: drag.size * 0.44, textShadow: '0 2px 6px rgba(0,0,0,0.25)' }}>
              {dragDef.letter}
            </span>
            <span className="font-bold rounded-lg w-full text-center truncate px-1.5 py-0.5"
